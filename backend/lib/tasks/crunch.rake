namespace :crunch do
  task :council, [:youtube_link] => :environment do |t, args|
    ...
    date_response = client.chat(
    parameters: {
        model: "gpt-3.5-turbo",
        messages: [
          { role: "user", content: "Trouve la date dans le titre de la vidéo suivante et formate-là comme suit: YYYY-MM-DD. \n\n #{title}" }
        ],
        temperature: 0.0,
    })
    puts date_response.dig("choices", 0, "message", "content")
