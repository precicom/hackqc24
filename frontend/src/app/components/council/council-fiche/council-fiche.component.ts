...
export class CouncilFicheComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    combineLatest([this.selectedThemes$, this.search$, this.discussionPoints$])
      .pipe(untilDestroyed(this))
      .subscribe(([selectedThemes, search, discussionPoints]) => {
        ...
        if (search) {
          filteredPoints = filteredPoints.filter(point =>
            `${point.generated_summary} ${point.theme.category} ${point.theme.name}`.toLowerCase().includes(search.toLowerCase()),
          )
        }
